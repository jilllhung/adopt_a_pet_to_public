...
const PetList = (props) => {
	...
	useEffect(() => {
		...
		let x = async () => {
			try {
				let pls = await axios.get(
					`http://localhost:8080/pets/${props.spec}`
				);
				let bls = await axios.get(
					`http://localhost:8080/breeds/species/${props.spec}`
				);
				console.log(pls);
				console.log(bls);
				if (loaded) {
              ...
              (pet.ageGrp&&pet.ageGrp.name.toLowerCase()===age)
					...
					.filter(
						(pet) =>
							breed === "" ||
							(pet.breedPrimary &&
								pet.breedPrimary.name === breed) ||
							(pet.breedSecondary &&
								pet.breedSecondary.name === breed)
					)
					.map((pet, i) => (
						<Pet key={i} pet={pet} />
					))}
